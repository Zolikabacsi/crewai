#!/usr/bin/env python3
"""COO Daemon — listens on AgentBus, handles process design, capacity planning, and RACI mapping.

Listens as 'COO', handles 'coo_request' and 'ping' actions,
calls run_coo_task() which routes to the right sub-agent (ProcessDesigner/CapacityPlanner/RACIMapper),
saves results to vault, and replies.

Vault path: /home/zoltan/srv/vault/Second Brain/raw/Aestas_COO/
"""
...
from src.agents.coo import run_coo_task

# ── Paths ─────────────────────────────────────────────────────────────────────
VAULT_ROOT = Path("/home/zoltan/srv/vault/Second Brain/raw/Aestas_COO")
LOG_PATH = Path("/home/zoltan/logs/coo_daemon.log")
...
logger = logging.getLogger("COODaemon")


def main() -> None:
    """Start the COO daemon on AgentBus."""
    logger.info("Starting COO daemon")
    ...
    bus.register(role=agent_name, metadata={"pid": os.getpid(), "type": "coo"})
    logger.info("Registered as '%s' on AgentBus", agent_name)

    def shutdown(signum, frame) -> None:
        logger.info("Shutting down COO daemon on signal %d", signum)
        sys.exit(0)
    ...
    while True:
        ...
        if action == "coo_request":
            ...
            if not question:
                logger.warning("coo_request with empty question from %s", from_agent)
                bus.send(
                    to=from_agent,
                    from_=agent_name,
                    action="coo_result",
            ...
            logger.info("Processing coo_request: %s", question[:100])
            ...
            with AgentLogger(agent="coo", task_name=question, vault_output_path=VAULT_ROOT) as run_log:
                run_log.step("task_received", {"question_preview": question[:100], "domain": domain})
                try:
                    run_log.step("coo_task_start")
                    result = run_coo_task(question)
                    ...
                    run_log.finish(status="failed", summary=f"COO task failed: {e}")
                    bus.send(to=from_agent, from_=agent_name, action="coo_result", data={
